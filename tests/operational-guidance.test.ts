...
function fixtureEvents(scenario: ConcreteScenario): object[] {
  ...
  if (scenario === "schedule-due-storm-isolation") return [{ event: "schedule_due_storm_isolation_complete", definitionsCreated: 512, deliveries: 512, canariesAttempted: 4, canariesCompleted: 4, readinessChecks: 4, postStormCanaries: 1, elapsedMs: 1_000 }];
  ...
  if (scenario === "same-shard-family-failure-isolation") return [{ event: "same_shard_family_failure_isolation_complete", shardCount: 4, failedFamily: 1, siblingFamily: 5, failedFamilyRejections: 2, siblingOperations: 2, readinessChecks: 2, crossFamilyDeliveries: 0, elapsedMs: 1_000 }];
  ...
  if (scenario === "family-actor-inflight-concurrent-failure") return [{ event: "family_actor_inflight_concurrent_failure_complete", cycles: 3, concurrentFailures: 12, streamInflightRejections: 6, rpcInflightTerminations: 6, siblingOperations: 6, readinessChecks: 6, metricIncrements: 12, restartRecoveries: 3, crossFamilyDeliveries: 0, elapsedMs: 1_000 }];
  return [];
}

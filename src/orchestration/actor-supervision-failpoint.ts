...
export function assertActorSupervisionEvidence(record: Readonly<Record<string, unknown>>): void {
  if (record.domainsInjected !== 6) throw new Error(`actor failpoints injected for ${String(record.domainsInjected)}/6 domains`);
  if (record.readinessWithdrawals !== 6) throw new Error(`readiness withdrawals ${String(record.readinessWithdrawals)}/6`);
  if (record.restartsRecovered !== 6) throw new Error(`restart recoveries ${String(record.restartsRecovered)}/6`);
...
export async function runActorSupervisionFailpointScenario(stack: ComposeStack, config: RunConfig, shape: WorkloadShape, artifacts: Artifacts): Promise<void> {
  ...
  const evidence = { domainsInjected, readinessWithdrawals, restartsRecovered, canaryDeliveries: expectedCanaryDeliveries, expectedCanaryDeliveries, queueRecovered: expectedQueueRecovered, expectedQueueRecovered, kvRecovered, leaseRecovered, scheduleRecovered, streamRecovered };
...
async function injectAndRestart(domain: "notice" | "queue" | "kv" | "lease" | "schedule" | "stream", stack: ComposeStack, config: RunConfig): Promise<void> {
